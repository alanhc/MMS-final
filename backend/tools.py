...
def get_nft(address, query_sting=""):
    ...
    result = []
    for token in  nfts.get("tokens"):
        
        nft = {}
        nft["tokenId"] = token.get("tokenId")
        nft["contract"] = token.get("contract")
        nft["owners"] = token.get("owners")
        nft["thumbnailUri"] = token.get("thumbnailUri").replace("ipfs://", "https://assets.akaswap.com/ipfs/")
        result.append(nft)
    return result
